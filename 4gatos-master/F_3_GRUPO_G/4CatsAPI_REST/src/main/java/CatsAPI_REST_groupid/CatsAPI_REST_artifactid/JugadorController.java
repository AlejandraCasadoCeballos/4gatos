...
public class JugadorController {
  ...
  public ResponseEntity<Jugador> actualizaJugador(@PathVariable long id, @RequestBody Jugador jugadorActualizado) {
    ...
    if (jugador != null) {
      ...
      //de aqui para abajo deberia estar en la otra clase

      jugadorActualizado=jugador.salaEspera(jugadorActualizado);
  ...
  public ResponseEntity<Jugador> borraJugador(@PathVariable long id) {
    ...
    Jugador jugador=jugadores.get(id); 
    jugador.setFueraSala(true);
    jugador.setEnSala(false);
    jugador.setParejaEncontrada(false);
    jugadores.put(id, jugador);
    jugador=jugadores.remove(id); 
    if (jugador != null) {
      return new ResponseEntity<>(jugador, HttpStatus.OK);
    } else {
      return new ResponseEntity<>(HttpStatus.NOT_FOUND);
