...
Además, el hecho de haberla tenido que hacer en el puente (lo cual nos ha complicado juntarnos para trabajar) y el hecho de que teníamos más prácticas y exámenes que hacer, no nos ha permitido dedicarle todo el tiempo que nos hubiese gustado a esta. 

Por último, nos ha costado mucho entender la teoría de cliente servidor y eso ha provocado que fuésemos mucho más lentos a la hora de programar lo que se pedía, de esta manera solo hemos conseguido hacer el requisito mínimo que era saber cuándo un jugador está conectado o no.
...
• Al pulsar uno de los dos gatos que puedes elegir, se realiza un POST de manera que dependiendo del gato elegido, al jugador se le asigne el nombre de este (es decir, el nombre puede ser Muffin o Mungojerry).
...
Para iniciar el juego desde eclipse es necesario tenerlo descargado. Una vez tenemos Eclipse lo primero que se debe hacer una vez abierto Eclipse es abrir el proyecto eligiendo la opción de abrir proyecto desde archivo, y escoger el directorio donde se ha guardado el proyecto tras haberlo descomprimido. Una vez abierto el proyecto, se debe abrir el archivo pom.xml y cuando se abra, hacer clic derecho sobre él, elegir "Maven" y "Update Project...". Una vez hecho esto, se vuelve a hacer clic sobre él y se elige "Run As" y "Maven Build". Si se abre una ventana, se debe escribir en el apartado "goals" lo siguiente: "clean test". Por último, se debe abrir el archivo "Application.java" y haciendo clic derecho sobre él, elegir "Run As" y "Java Application". De esta manera el servidor debería estar corriendo ya. 
## Iniciar el juego desde Spring Tool Suite
A la hora de iniciar el juego desde Spring Tool Suite es necesario descargarlo. Una vez lo tenemos, es necesario importar el proyecto (primero hay que extraerlo ya que la entrega es un zip). Para ello hay que darle a "file" y tras ello a darle a importar. En este momento hay que seleccionaar la opción de "existing maven project" que está en la carpeta llamada "Maven". Tras ello hay que darle a "browse" y seleccionar la carpeta del proyecto. Para terminar de importarlo hay que darle a "finish".
Una vez importado el juego en STS, hay que dar click derecho sobre la carpeta creada, seleccionar "run as" y darle a "Spring Boot App". De esta manera ya tenemos activado el servidor.
### El juego
...
-Ademas de detectar la inactividad desde el servidor (importante para casos en los que el jugador cierre la pantalla del juego), detectamos la inactividad desde el cliente (lo cual nos sirve para casos en los que el cliente sigue con la pestaña abierta pero se ha ido a otra o no está usando el ordenador). En estos casos se hace un delete del jugador y se le envía al menú del juego, que ahora es la primera pantalla.
-Tambien existe un delete cuando el jugador decide ir al menú desde la pausa.
...
Esto en la fase 5: En cuanto a las mejoras del juego, tras hacer beta testing nos dimos cuenta de que había algunos bugs que arreglar y aspectos estéticos que mejorar, en concreto los siguientes:
